...
def main():
    ...
    with st.sidebar:
        ...
        uploaded_jd_file = st.file_uploader("Ou uploader une description de poste", 
                                          type=['txt', 'pdf', 'docx'])
        ...
        if uploaded_jd_file:
            try:
                if uploaded_jd_file.type == "application/pdf":
                    ...
                    job_description = " ".join([page.extract_text() for page in reader.pages if page.extract_text()])
                
                elif uploaded_jd_file.type == "application/vnd.openxmlformats-officedocument.wordprocessingml.document":
                    doc = Document(uploaded_jd_file)
                    job_description = " ".join([para.text for para in doc.paragraphs if para.text])
        ...
        # Upload des CVs
        uploaded_cvs = st.file_uploader("Uploader les CVs (PDF/DOCX)", 
                    ...
                    try:
                        ...
                        if cv_text:
                            ...
                            results.append({
                                ...
                                "Score": score,
                                "Match %": match_percent,
                                "Points forts": "Expérience pertinente" if score > 50 else "Compétences de base",
                                "Points faibles": "Manque d'expérience" if score <= 50 else "Peu d'infos"
                            })
